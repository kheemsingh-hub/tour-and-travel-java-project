...
public class BookHotel extends JFrame implements ActionListener {
	...
	public void actionPerformed(ActionEvent ae) {
		if(ae.getSource()==checkprice) {
			...
			try {
				Conn c=new Conn();
				c.s.executeUpdate("insert into bookhotel value('"+labelusername.getText()+"', '"+chotel.getSelectedItem()+"','"+tfperson.getText()+"','"+tfdays.getText()+"','"+cac.getSelectedItem()+"','"+cfood.getSelectedItem()+"','"+labelid.getText()+"','"+labelnumber.getText()+"','"+labelphone.getText()+"','"+labelprice.getText()+"')");
	...
	public static void main(String[] args) {
	    try {
	        Conn c = new Conn();
	        ResultSet rs = c.s.executeQuery("SELECT username FROM bookhotel LIMIT 1"); // or use FETCH FIRST 1 ROWS ONLY for some DBs
	        if (rs.next()) {
